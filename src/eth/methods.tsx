...
export async function getStakedBalance(address: string, pool: Pool, provider: any) {
  const contract = new ethers.Contract(pool.address, pool.abi, provider);
  const result = formatResult(await contract.balanceOf(address), pool.token.decimals);
  return result;
}
...
export async function boostV4(level: number, pool: Pool, token: Token, signer: any) {
  toast(<AppToast spinnerCondition={true} txId={""} message="Waiting boost confirmation" />, {
    position: "top-left",
    autoClose: 2000,
    hideProgressBar: false,
...
export async function getBoostAllowance(address: string, pool: Pool, boostToken: Token, signer: any) {
  if (boostToken) {
    ...
    const contract = new ethers.Contract(boostToken.address, boostToken.abi, signer);
