...
export async function getCurrentTotalStake(
  address: string,
  pool: Pool,
  provider: any
) {
...
export async function stake(
  ...
  toast(
    <AppToast
      ...
      txId={""}
      message="🥱 Pending stake transaction confirmation"
    />,
    {
      position: "top-left",
      autoClose: 2000,
  ...
  toast(
    <AppToast spinnerCondition={true} txId={tx.hash} message="🔮 Staking.. " />,
    {
      position: "top-left",
      autoClose: false,
      hideProgressBar: false,
...
export async function maxStake(
  pool: Pool,
  signer: any,
  account: string,
  provider: any
  ...
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="🥱 Pending maximum stake confirmation."
    />,
    {
      position: "top-left",
      autoClose: 2000,
...
export async function withdraw(
  ...
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="🥱 Pending withdrawal confirmation."
    />,
    {
      position: "top-left",
      autoClose: 2000,
...
export async function maxWithdraw(
  ...
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="🥱 Pending max withdrawal confirmation."
    />,
    {
      position: "top-left",
      autoClose: 2000,
  ...
  toast(
    <AppToast
      spinnerCondition={true}
      txId={tx.hash}
      message="✅ Max withdrawal complete"
    />,
    {
      position: "top-left",
      autoClose: 3000,
...
export async function claim(pool: Pool, signer: any) {
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="🥱 Pending claim confirmation"
    />,
    {
      position: "top-left",
      autoClose: 2000,
...
export async function exit(pool: Pool, signer: any) {
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="🥱 Pending exit confirmation"
    />,
    {
      position: "top-left",
      autoClose: 2000,
  ...
  toast(
    <AppToast spinnerCondition={true} txId={tx.hash} message="🔮 Exiting.." />,
    {
      position: "top-left",
      autoClose: false,
      hideProgressBar: false,
...
export async function checkNormalAllowance(
  address: string,
  target: Token,
  account: string,
  provider: any
...
export async function getBoostCosts(
  address: string,
  pool: Pool,
  provider: any
) {
  if (pool.boostToken && pool.boostLevels) {
    ...
    const promises = pool.boostLevels.map(({ level }) =>
      contract.calculateCost(level)
    );

    const costs = await Promise.all(promises);

    const decimals = pool.boostToken.decimals;
    const results = costs.map(
      (levelCost) =>
        formatResult(levelCost, decimals) - formatResult(tokensSpent, decimals)
    );
    return results;
  }
}
...
export async function boost(level: number, pool: Pool, signer: any) {
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="Waiting boost confirmation"
    />,
    {
      position: "top-left",
      autoClose: 2000,
  ...
  toast(
    <AppToast spinnerCondition={true} txId={tx.hash} message="🔮 Boosting.." />,
    {
      position: "top-left",
      autoClose: false,
      hideProgressBar: false,
...
export async function boostV4(
  ...
  const boostToast = "boostToast";
  const contract = new ethers.Contract(pool.address, pool.abi, signer);
  let tx = await contract.purchase(token.address, level);
  toast(
    <AppToast spinnerCondition={true} txId={tx.hash} message="🔮 Boosting.." />,
    {
      position: "top-left",
...
export async function getBoostLevel(
  address: string,
  pool: Pool,
  provider: any
) {
...
export async function checkBoostAllowance(
  ...
  if (pool.boostToken) {
    const contract = new ethers.Contract(
      pool.boostToken.address,
      pool.boostToken.abi,
      provider
    );
    return (
      formatResult(
        await contract.allowance(address, pool.address),
        pool.token.decimals
      ) > 0
    );
  } else {
...
export async function getTokensSpentOnBoost(
  address: string,
  pool: Pool,
  provider: any
) {
  if (pool.boostToken) {
    const contract = new ethers.Contract(pool.address, pool.abi, provider);
    return formatResult(
      await contract.getSpent(address),
      pool.boostToken.decimals
    );
  }
}
...
export async function getBoostMultiplier(
  ...
  const contract = new ethers.Contract(pool.address, pool.abi, provider);
  const multi = await contract.getTotalMultiplier(address);
  return pool.v4 ? Number(multi) : formatResult(multi, 18);
}

export async function getBoostAllowance(
  address: string,
  pool: Pool,
  boostToken: Token,
  signer: any
  ...
  if (boostToken) {
    toast(
      <AppToast
        spinnerCondition={true}
        txId={""}
        message="🥱 Pending boost token approval"
      />,
      {
        position: "top-left",
        autoClose: 2000,
    ...
    toast(
      <AppToast
        spinnerCondition={true}
        txId={tx.hash}
        message="✅ Boost token approval complete"
      />,
      {
        position: "top-left",
        autoClose: 3000,
...
export async function getAllowance(pool: Pool, signer: any) {
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message="🥱 Pending allowance confirmation"
    />,
    {
      position: "top-left",
      autoClose: 2000,
  ...
  toast(
    <AppToast
      spinnerCondition={true}
      txId={tx.hash}
      message="✅ Token approval complete"
    />,
    {
      position: "top-left",
      autoClose: 3000,
...
export async function getAllowanceQuick(
  token: Token,
  target: string,
  signer: any
) {
...
export async function swapV1toV2(token: Token, signer: any) {
  toast(
    <AppToast
      spinnerCondition={true}
      txId={""}
      message={`🥱 Pending swap ${token.name} confirmation`}
    />,
    {
      position: "top-left",
      autoClose: 2000,
  ...
  toast(
    <AppToast spinnerCondition={true} txId={tx.hash} message="🔮 Swapping.." />,
    {
      position: "top-left",
      autoClose: false,
      hideProgressBar: false,
...
export async function getRewardsAvailable(
  ...
  const contract = new ethers.Contract(pool.address, pool.abi, provider);
  const tokens = formatResult(
    await contract.earned(address),
